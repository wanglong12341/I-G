...
@ddt.ddt
class api_query_withdraw(unittest.TestCase):
	excel = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + Config().Get_Item('File','51_case_file')
	excel_data = excel_table_byname(excel, 'api_query_withdraw')
	def setUp(self):
		self.cm = Common()
		self.logger = Logger(logger="api_query_withdraw").getlog()

	def tearDown(self):
		pass

	@ddt.data(*excel_data)
	def test_api_query_withdraw(self,data):
