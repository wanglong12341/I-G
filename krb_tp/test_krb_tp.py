...
class credit_apply(unittest.TestCase):
	...
	def test_2_personal_regist_expand(self):
		...
		rep = self.cm.form_request(faceaddr=data[0]['url'], headers=headers,
								   param=params, product='krb')
		with open(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/data/cg.html','w',encoding='utf-8') as f:
			f.write(rep.text)
		...
		if system == 'mac':
			...
			print("不支持该系统!")
		url = "file://" + os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/data/cg.html'
		driver.get(url)
		time.sleep(1)
		try:
			...
			driver.find_element_by_css_selector("html body div.container.min-width.containerMine div.content div.inputCon form#form.registerform div.pd-32 div.checkboxCon div.checkbox.fc label.gray i.icon-check img").click()
